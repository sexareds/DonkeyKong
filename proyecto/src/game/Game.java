 ...
 * 
 * @author Jonas Aray, Jesus Betancourt, Teresa Youssef, Cesar Rojas
 * 
 */
...
public class Game extends Canvas implements Runnable {
    ...
    /**
     * Actualizar el estado del teclado y el jugador
     */
    private void tick() {
        keyboard.tick();
        player.tick();
    ...
    private void render() {
        ...
        screen.clear();
        level.render(player.getX() - (screen.getWidth() - player.getSprite().getSide()) / 2, player.getY() - (screen.getWidth() - player.getSprite().getSide()) / 2, screen, (Graphics2D)g);
        player.render(screen, (Graphics2D)g);
        System.arraycopy(screen.pixels, 0, pixels, 0, pixels.length);
        fps++;
    }

    /**
     * Metodo para correr el juego, contiene el reloj que se encarga
     * de actualizar y renderizar. 
     * 
     * Lleva la cuenta de los UPS y FPS
     */
    @Override
    public void run() {
    ...
    public static void main(String[] args) {
        Menu obj = new Menu();
        obj.setTitle("Menu Java");
        obj.setSize(400,200);
        obj.setVisible(true);
        new Game().start();
        new Sound().playMusic("bacmusic.wav");
    }
}
